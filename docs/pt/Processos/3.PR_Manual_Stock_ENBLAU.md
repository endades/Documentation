...
> ℹ️ **Nota:** Para mais informações sobre como definir mínimo, máximo e estoque habitual, consulte [Configuração de materiais](#51-configuracion-de-materiales).
...
### 3.4. Relatórios de armazéns (inventário)

- Na seção **Armazéns** - **Estoque** é possível imprimir diferentes tipos de relatórios:

  - **Resumo:** Existem dois relatórios: **Inventário** e **Inventário por data**.

    ![Listado informes](Imagenes/PR_Stock_Control/informes.jpg)

    **Inventário** mostra um relatório de inventário, de acordo com os filtros aplicados na lista de armazém. Inclui as colunas Artigo, Projeto, Unidades, Unidades pendentes e Unidades reservadas.

      ![Inventário](Imagenes/PR_Stock_Control/inventario.jpg)

    **Inventário por data** mostra um relatório de inventário na data selecionada. Inclui as colunas Artigo, Descrição, Unidades e Valor.

      ![Inventário por data](Imagenes/PR_Stock_Control/inventario_fecha.jpg)

  - **Resumo com localização:** Existe um relatório **Inventário detalhado**.

    **Inventário detalhado** mostra um relatório de inventário, de acordo com os filtros aplicados na lista detalhada de armazém. Inclui Localização no armazém, Artigo, Descrição e Unidades.

      ![Inventário detalhado](Imagenes/PR_Stock_Control/inventario_detallado_resumen.jpg)

  - **Detalhado:** Existe um relatório **Inventário detalhado**.

    **Inventário detalhado** mostra um relatório de inventário, de acordo com os filtros aplicados na lista detalhada de armazém. Inclui Projeto, Número da guia de compra, Localização no armazém, Artigo, Descrição e Unidades agrupadas por referência e comprimento.

      ![Inventário detalhado](Imagenes/PR_Stock_Control/inventario_detallado.jpg)

  - Na seção **Armazéns** - **Movimentos de armazém** é possível imprimir diferentes tipos de relatórios:

    ![Listado informes](Imagenes/PR_Stock_Control/informes_movimientos.jpg)

    **Movimentos** mostra uma lista dos movimentos visualizados na lista de movimentos. Inclui Data do movimento, Responsável, Projeto atribuído, Localização no armazém, Artigo, Descrição, Unidades e Tipo de movimento.

      ![Listado informes](Imagenes/PR_Stock_Control/informes_movimientos2.jpg)

    **Movimentos I** mostra uma lista dos movimentos visualizados na lista de movimentos. Inclui as colunas Tipo de movimento, Data do movimento, Responsável, Documento, Unidades, Valor, Artigo e Descrição.
...
- **Comprar**:  
  Se esta caixa estiver marcada, indica que o material será proposto por padrão como “a comprar” quando listado na seção **Material necessário**.

- **Montado no destino**:  
  Se marcada, o material será montado no destino durante o processo no posto **Picking** dentro do **enCONTROL**.

- **Controle de estoque**:  
  Quando ativado, o material será considerado para desconto automático durante o processo de produção.
...
## 8. Controle de estoque e relatórios
