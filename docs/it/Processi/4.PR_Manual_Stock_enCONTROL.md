# Sistema di controllo delle scorte enCONTROL

---

## 1. Scopo

Questo manuale è progettato per i nuovi utenti del sistema di controllo delle scorte utilizzato dai Clienti ed Endades. L'obiettivo principale è fornire una guida dettagliata per la gestione efficiente dei progetti, dalla creazione degli ordini all'ottimizzazione del processo produttivo in enCONTROL ed enSITE. Questo documento copre i passaggi necessari per eseguire le operazioni chiave del sistema ed è strutturato in modo sequenziale per facilitarne la comprensione.

---

## 2. Ricezione dei materiali
### 2.1. Ricezione in enSITE

- Una volta creato l'ordine di acquisto, da ENBLAU in **Magazzini**, accedere all'opzione **"In attesa di ricevere"** nella sezione Magazzini. Si aprirà una scheda con tutti i materiali in attesa di essere ricevuti per ogni progetto.

![pendientes_recibir](Imagenes/PR_Stock_Control/pendiente_recibir.jpg)
![pendientes_recibir](Imagenes/PR_Stock_Control/pendiente_recibir2.jpg)

> **Nota:** Per informazioni più dettagliate sugli ordini di acquisto, segui questo link: [2.1. Ordini di acquisto](3.PR_Manual_Stock_ENBLAU.md#21-ordini-di-acquisto)

- **Modalità di ricezione**: Aprire **enSite** dall'officina tramite wifi con la persona incaricata di ricevere l'ordine.
- In **Ricezione ordini**, individuare l'ordine corrispondente (ordinato per data).

![recepcion_pedido](Imagenes/PR_Stock_Control/recepcion_pedido.jpg)
![recepcion_pedido](Imagenes/PR_Stock_Control/recepcion_pedido2.jpg)

- Registrare la ricezione in **Nuova bolla di consegna** creando una bolla dalla sezione **Magazzino**.

![albaran](Imagenes/PR_Stock_Control/albaran.jpg)
![albaran](Imagenes/PR_Stock_Control/albaran2.jpg)
![albaran](Imagenes/PR_Stock_Control/albaran3.jpg)

- In ENBLAU, la scheda **In attesa di ricevere** di **Magazzini** si sincronizza e non vedrai più i materiali per quell'ordine.
- In **Ordini di Acquisto**, puoi verificare la tracciabilità.
- In **enSite**, indicare le quantità ricevute e l'ubicazione dove verranno stoccati (Magazzino, Piano, Zona, Sottozona...). Successivamente potrai vedere l'ubicazione di ogni materiale.

![ubicacion](Imagenes/PR_Stock_Control/ubicacion.jpg)
![ubicacion](Imagenes/PR_Stock_Control/ubicacion2.jpg)

- Dall'app **enSITE**, è anche possibile cercare per articoli e vedere tutte le ubicazioni dove si trova quell'articolo. Per poi spostare o scalare gli articoli.

    ![ver_ubicacion](Imagenes/PR_Stock_Control/ver_ubicacion.jpg) 
    ![mover_salida](Imagenes/PR_Stock_Control/mover_salida.jpg)

> **In caso di materiali danneggiati o altre anomalie**, puoi allegare documenti o scattare foto e allegarle alla bolla. Poi potrai rivederle da ENBLAU.

![adjuntos](Imagenes/PR_Stock_Control/adjuntos.jpg)
![adjuntos](Imagenes/PR_Stock_Control/adjuntos2.jpg)


### 2.2. Ricezione in ENBLAU

- Da ENBLAU, una volta creato l'ordine di acquisto, è possibile generare una bolla di ricezione materiali dell'ordine.

- Dalla bolla potrai indicare la ricezione di ogni materiale specificando la quantità. Per impostazione predefinita saranno tutti marcati con la quantità dell'ordine.

- Successivamente occorre indicare l'ubicazione della bolla nella scheda magazzini della bolla.

> **Nota:** Per informazioni più dettagliate sulla ricezione dei materiali in ENBLAU, segui questo link: [4.2. Ricezione del materiale](1.PR_Ventas_Compras.md#42-ricezione-del-materiale)

---

## 3. Produzione e enCONTROL

### 3.1. Produzione

- Crea progetti di produzione basati sugli ordini cliente in ENBLAU.
  - Creare produzione.

    ![crear_produccion](Imagenes/PR_Stock_Control/crear_produccion.jpg)

  - Il passo successivo è **Invia in produzione**.

    ![enviar_produccion](Imagenes/PR_Stock_Control/enviar_produccion.jpg)
    

  - In produzione, si può vedere in **scorta necessaria** i materiali scalati e si può scalare manualmente.

    ![stock_necesario](Imagenes/PR_Stock_Control/stock_necesario.jpg)
    ![stock_necesario](Imagenes/PR_Stock_Control/stock_necesario2.jpg)

> **Nota:** Per informazioni più dettagliate sulla creazione della fase di produzione in ENBLAU, segui questo link: [3.5. Produzione](1.PR_Ventas_Compras.md#35-produzione)

### 3.2. enCONTROL

- Apri **enCONTROL**. Appaiono i progetti di produzione. Se sei un utente amministratore, vedrai tutti i progetti, altrimenti solo quelli assegnati.

- Apri il **monitor di controllo**. Puoi aprirlo facendo doppio clic su uno qualsiasi dei filtri (nell'intestazione) o facendo doppio clic sul progetto e selezionando la postazione **Controllo** se l'hai configurata nel computer.

    ![puesto_control](Imagenes/PR_Stock_Control/control.jpg)

- Quando si invia in produzione (Officina), viene messo in coda di produzione nella scheda **Gestione Produzione**. Il responsabile di produzione decide la priorità.

    ![gestion_produccion](Imagenes/PR_Stock_Control/gestion_produccion.jpg)

- Per togliere da "In coda", basta trascinarlo nel pannello "In produzione" e si aprirà una finestra **Attività** per proporre una data, indicare lo stato, la classificazione, ecc.

    ![cola](Imagenes/PR_Stock_Control/cola.jpg)

    ![tarea](Imagenes/PR_Stock_Control/tarea.jpg)

- Nella scheda Pianificazione produzione definisci a chi viene assegnato, trascinando i progetti a ciascun responsabile. Devi selezionare il responsabile e trascinare il progetto (si possono selezionare più responsabili con ctrl + tasto destro). Si può anche cambiare la priorità da sinistra a destra e dall'alto in basso:

  ![planificacion_produccion](Imagenes/PR_Stock_Control/planificacion_produccion.jpg)

  Per rimuovere il progetto assegnato, devi selezionare il responsabile, selezionare il progetto e usare ctrl + canc.

- Nella scheda Calendario produzione viene mostrato un diagramma di Gantt basato sui tempi di Logikal (nell'esempio) date di ingresso e uscita dalla produzione.

  ![calendario_produccion](Imagenes/PR_Stock_Control/calendario_produccion.jpg)

- Ottimizza i processi secondo le aree di produzione (taglio, lavorazione meccanica, assemblaggio, ecc.). Nella schermata principale del progetto con doppio clic sul progetto desiderato, si aprirà una finestra Seleziona una postazione. (Le postazioni sono definite come decide il cliente e si vedranno in base all'utente) esempi:

  ![seleccion_puesto](Imagenes/PR_Stock_Control/seleccion_puesto.jpg)

- Entrando in qualsiasi postazione si aprirà la schermata con il pulsante **Avvia**:

  ![iniciar](Imagenes/PR_Stock_Control/iniciar.jpg)

  All'avvio viene conteggiato il tempo fino a quando si mette in pausa o si termina il lavoro.

- Tutte le schermate hanno alcuni campi in comune:
    - Commento - Si può aggiungere un commento e scegliere una gravità nel menu a tendina. L'ultimo commento positivo è quello che sblocca.

      ![comentario](Imagenes/PR_Stock_Control/comentario.jpg)

      ![comentario](Imagenes/PR_Stock_Control/comentario2.jpg)

    - Pausa - Mettendo in pausa il lavoro ti obbliga a indicare dall'elenco il motivo della pausa. Poi si può riprendere indicando di nuovo la stessa postazione.

      ![pausar](Imagenes/PR_Stock_Control/pausar.jpg)

    - Termina - Si termina quando finisci il lavoro in quella postazione. Poi si può riprendere indicando di nuovo la stessa postazione.

      ![finalizar](Imagenes/PR_Stock_Control/finalizar.jpg)

    - Documentazione - Dalla sezione documentazione si potranno vedere i documenti di produzione esportati da Logikal nel visualizzatore di documentazione. Si può anche aggiungere un percorso generico per aggiungere quello che si vuole. Catalogo, manuali, ecc.

      ![documentacion](Imagenes/PR_Stock_Control/documentacion.jpg)

#### 3.2.1. Postazioni di lavoro

- Con un doppio clic sulla produzione si aprirà la finestra della postazione di lavoro configurata per ogni squadra.

    ![puesto_control](Imagenes/PR_Stock_Control/puesto_control.jpg)

 **1. Preparazione Materiale (Postazione 0)**

  - Tutto Preparato – Contrassegna i materiali con un segno di spunta verde indicando che sono preparati.
  - Nulla Preparato – Contrassegna i materiali con una X rossa indicando che non sono preparati.
  - Ubicazione Materiale – Come configurato inCONTROL, potrebbe indicare dove posizionare i materiali. In modo che quel materiale venga scalato solo da quell'ubicazione.

      ![preparacion](Imagenes/PR_Stock_Control/preparacion.jpg)

**2. Taglio (Postazione 1)**

  - In taglio si indica la lunghezza della barra e lo scarto dei ritagli. Si apre una finestra dei ritagli e si può cambiare la lunghezza secondo necessità.

    ![corte](Imagenes/PR_Stock_Control/corte.jpg)

    ![desperdicio](Imagenes/PR_Stock_Control/desperdicio.jpg)

    > ℹ️ **Nota:** Da ENBLAU nella sezione Configurazione - Generale - **Logikal**. Si può indicare il **Ritaglio minimo** da considerare quando si taglia una barra nella postazione di **Taglio**.

      ![retales](Imagenes/PR_Stock_Control/retales.jpg)

      ![retales](Imagenes/PR_Stock_Control/retales2.jpg)

  - Poi da ENBLAU **Movimenti** di magazzino si rifletterà un'uscita della barra totale es. 6500 e poi un'entrata della stessa materia di es. 4100 come indicato nella postazione di taglio.

    ![movimientos_barra](Imagenes/PR_Stock_Control/movimientos_barra.jpg)

    - Anche in ENBLAU si può vedere nella sezione **Stock** l'elenco filtrato per **È ritaglio** tutti i ritagli disponibili in stock.

      ![retales](Imagenes/PR_Stock_Control/retales3.jpg)

  - Nella sezione **Tagliati** della postazione taglio di enCONTROL nella parte destra del monitor viene mostrato l'elenco dei tagli effettuati e si possono stampare le etichette di ogni barra.

    ![cortados](Imagenes/PR_Stock_Control/cortados.jpg)

**3. Lavorazione (Postazione 2)**

  - Non scala materiali, è solo informativo per i tempi. Confronta il tempo reale.

    ![mecanizado](Imagenes/PR_Stock_Control/mecanizado.jpg)

**4. Montaggio (Postazione 3)**

  - Scala materiali (accessori) per ogni posizione di telaio.

    ![montaje](Imagenes/PR_Stock_Control/montaje.jpg)

  - Nell'elenco materiali hai la possibilità di indicare gli articoli che verranno montati a destino. Quei materiali verranno poi aggiunti all'elenco della postazione **Picking**.

  ![montaje_2](Imagenes/PR_Stock_Control/montaje_2.jpg)

**5. Guarnizionatura (Postazione 4)**

  - Scala materiali (Guarnizioni, calzature, schiuma, ecc.) per telaio.

  ![ajunquillado](Imagenes/PR_Stock_Control/ajunquillado.jpg)

  - Nell'elenco materiali hai la possibilità di indicare gli articoli che verranno montati a destino. Quei materiali verranno poi aggiunti all'elenco della postazione **Picking**.

    ![ajunquillado_2](Imagenes/PR_Stock_Control/ajunquillado_2.jpg)

**6. Assemblaggio (Postazione 5)**

  - Informazioni del telaio, ma non scala materiali.

  ![ensamblado](Imagenes/PR_Stock_Control/ensamblado.jpg)

**7. Ferramenta (Postazione 6)**

  - Scala materiali (ferramenta) per anta.

    ![herraje](Imagenes/PR_Stock_Control/herraje.jpg)

  - Nell'elenco materiali hai la possibilità di indicare la ferramenta che verrà montata a destino. Quei materiali verranno poi aggiunti all'elenco della postazione **Picking**.

    ![herraje_2](Imagenes/PR_Stock_Control/herraje_2.jpg)

**8. Saldatura (Postazione 7)**

  - Non scala materiali, è solo informativo per i tempi. Confronta il tempo reale.

    ![soldado](Imagenes/PR_Stock_Control/soldado.jpg)

**9. Tenda/avvolgibile (Postazione 8)**

  - Scala materiali (tende/avvolgibili) per telaio.

    ![puesto_persiana](Imagenes/PR_Stock_Control/puesto_persiana.jpg)

**10. Trasformazione (Postazione 9)**

  - In questa postazione puoi indicare un gruppo di materiali definite in distinta base per trasformare in un prodotto, esempio: Ringhiera.

    ![transformacion](Imagenes/PR_Stock_Control/transformacion.jpg)

  - Scarica l’elenco dei materiali definito in ogni distinta base. Inoltre, se nella distinta base, da ENBLAU, è stato indicato un riferimento di materiale nel campo **Prodotto finito**, verrà generato un carico di magazzino con tale riferimento di articolo. Per maggiori informazioni, consulta il seguente link: [2.1. Creazione di Distinte base](/Utilidades/9.UT_Escandallos/)

  - Come in altre postazioni, nell'elenco materiali hai la possibilità di indicare gli articoli che verranno montati a destino. Quei materiali verranno poi aggiunti all'elenco della postazione **Picking**. Inoltre poi potrai stampare l'etichetta e ubicare il prodotto.

**11. Verifica Finale (Postazione 10)**

  - Non scala materiali. Ma, una volta terminata la verifica si può ubicare in Magazzino e poi stampare un'etichetta del prodotto finito.

    ![comprobacion](Imagenes/PR_Stock_Control/comprobacion.jpg)

    ![comprobacion_ubicacion](Imagenes/PR_Stock_Control/comprobacion_ubicacion.jpg)

    ![comprobacion_etiqueta](Imagenes/PR_Stock_Control/comprobacion_etiqueta.jpg)

  - Da ENBLAU nella sezione Magazzini nella sezione **Prodotto finito** ci sono due elenchi:

      **Ubicazione prodotti:** Mostra dove è ubicato il prodotto finito di ogni progetto e produzione. Da questo elenco si può dare uscita, spostare di ubicazione e aprire il documento di produzione, selezionando sulla posizione con il tasto destro.

      ![ubicacion_producto](Imagenes/PR_Stock_Control/ubicacion_producto.jpg)

      **Movimenti prodotti:** Mostra i movimenti del prodotto finito come entrata, trasferimento, uscita e reso. Da questo elenco si può fare un reso, selezionando sulla posizione di uscita con il tasto destro.

      ![movimientos_producto](Imagenes/PR_Stock_Control/movimientos_producto.jpg)

**12. Pulizia (Postazione 11)**

  - Non scala materiali, è solo informativo per i tempi. Confronta il tempo reale.

    ![limpieza](Imagenes/PR_Stock_Control/limpieza.jpg)

**13. Picking (Postazione 12)**

  - La postazione mostra l'elenco dei materiali contrassegnati precedentemente in altre postazioni che verranno montati a destino.

    ![picking](Imagenes/PR_Stock_Control/picking.jpg)

  - Puoi creare i pacchi e assegnare per ogni materiale. E poi stampare l'etichetta per il pacco.

    ![picking_paquete](Imagenes/PR_Stock_Control/picking_paquete.jpg)

    - Indicare l'ubicazione di picking per ogni materiale.

      ![picking_ubicacion](Imagenes/PR_Stock_Control/picking_ubicacion.jpg)

**14. Lavagna (Postazione 13)**

  - Mostra l'evoluzione di ogni progetto e il suo stato.

    ![pizarra](Imagenes/PR_Stock_Control/pizarra.jpg)

**15. Controllo (Postazione 14)**

  - Apre il monitor Controllo progetti. Dove si può anche vedere lo stato di ogni progetto che è in fabbrica.

    ![control_estados](Imagenes/PR_Stock_Control/control_estados.jpg)

> ℹ️ **Nota**: Assegnare priorità e risorse per assicurare la fluidità nella produzione.

> **Configurazione enCONTROL** - Configurare enCONTROL precedentemente. Per maggiori informazioni, segui questo link: [2. Configurazione Iniziale di enCONTROL](/Configurazioni/2.%20CO_Configurazione_Iniziale_enCONTROL/) :

> **Gestione filtri** - Usare filtri e filtri personalizzati per ottimizzare la ricerca negli elenchi. Per maggiori informazioni, segui questo link: [4. Gestione Filtri](/Utilita/4.UT_Gestione_Filtri/) :

---

## 4. Controllo scorte e report

- Monitora le scorte necessarie dalla sezione "Produzione".
- Identifica materiali riservati e in attesa in tempo reale.
- Genera report di inventario per valutare l'utilizzo dei materiali e pianificare futuri acquisti.

---

## 5. Risoluzione dei problemi comuni

- **Errore nell'importazione dei riferimenti**: Verifica che i riferimenti siano configurati nel database.
- **Materiali in eccesso o mancanti**: Assicurati di registrare correttamente le ubicazioni e le quantità quando ricevi i materiali.
- **Problemi con il layout del magazzino**: Rivedi la configurazione delle zone e regola secondo necessità.

---

## 6. Conclusione

Questo manuale offre una guida passo passo per i processi principali nel sistema di controllo delle scorte. Seguendo queste istruzioni, gli utenti potranno gestire efficientemente gli inventari, migliorare la produzione e ottimizzare il rapporto con i fornitori. Per supporto aggiuntivo, contattare l'amministratore del sistema.
