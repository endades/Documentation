...
> ⚠️ **¡Importante!** Asegúrate de que se puedan utilizar materiales del proyecto desde el que se desea hacer el traspaso de los artículos.
  ...
  - Columnas **Tiene stock en proyecto** y **Tiene stock en almacén**. Esas columnas se utilizan para filtrar los materiales que hay asignado al proyecto del documento de venta y los materiales que hay disponible en el proyecto definido para gestión de stock. De este modo habera un control de los materiales disponibles antes de emitir un pedido de compra al proveedor.
  
  ![Asignar articulos](Imagenes/PR_Stock_Control/asignar4.jpg)

  - Opción **Asignar material**. Esa función permite asignar de forma automática de materiales desde el proyecto que está definido como gestión de stock.
  
  ![Asignar articulos](Imagenes/PR_Stock_Control/asignar5.jpg)
...
- Los materiales **marcados en rojo** son aquellos que están por debajo del **mínimo** o del **habitual**. 
    ...
    - La columna **Unidades pendientes** muestra la cantidad pendiente de recibir.  
...
## 8. Gestión de retales en Logikal

La gestión de retales en Logikal permite reutilizar perfiles sobrantes, optimizando el consumo de material y reduciendo desperdicios. Existen diferente formas de gestionar los retales, que se detallan a continuación.

---

### 8.1. Importar retales desde Excel

1. Desde **ENBLAU**, acceder a **Almacenes → Stock → Detallado** y filtrar el listado por **retales**.

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales.jpg)

   > ℹ️ **Nota:** Se recomienda aplicar el filtro por **proyecto asignado** para considerar únicamente los retales que deban utilizarse.

2. En el menú **Exportar → Exportar perfiles**, guardar el archivo Excel en una ruta local de Windows.

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales2.jpg)

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales3.jpg)

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales4.jpg)

3. En **Logikal**, abrir el proyecto en el que se van a utilizar los retales. Acceder a **Proyecto → Retales** para abrir la ventana **Editar retales**.

4. Seleccionar la opción **Importar** y cargar el archivo Excel de retales previamente exportado desde ENBLAU.

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales5.jpg)

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales6.jpg)

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales7.jpg)

   > ℹ️ **Nota:** Las referencias mostradas en <span style= font-weight:bold;color:#C0392B;>rojo</span> indican que ese perfil no existe en el proyecto actual.

5. En la ventana **Editar retales**, marcar la opción **Usar retales** y guardar los cambios.

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales8.jpg)

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales9.jpg)

6. En el informe de **Optimización de cortes** se reflejarán los retales disponibles que pueden utilizarse para cada perfil.

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales10.jpg)

---

### 8.2. Añadir retales manualmente

Desde Logikal, en la ventana **Editar retales**, existe la opción **Añadir**, que permite introducir los retales de forma manual indicando:

- Referencia del perfil (existente en el proyecto)
- Cantidad
- Longitud

![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales1.jpg)

---

### 8.3. Gestionar retales desde ENBLAU

1. Desde **ENBLAU**, en un documento de ventas, acceder a  
   **Logikal → Editor de proyecto → Preferencias → Retales**.

   Esto abrirá la misma ventana **Editar retales** de Logikal.

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales11.jpg)

   > ℹ️ **Nota:** El funcionamiento es exactamente el mismo que el descrito previamente en Logikal.

2. Al listar el material desde el documento de ENBLAU, se mostrará la longitud del retal que se utilizará para los perfiles del proyecto.

   ![Gestión retales](Imagenes/PR_Stock_Control/utilizar_retales12.jpg)

   > ℹ️ **Nota:** Si el material está asignado por ejemplo, a un proyecto de **Stock**, es necesario asignarlo al proyecto en el que se va a utilizar.

---

## 9. Control de stock y reportes
