...
- Columns **Has stock in project** and **Has stock in warehouse**. These columns are used to filter the materials assigned to the sales document project and the materials available in the project defined for stock management. In this way, there will be control over the available materials before issuing a purchase order to the supplier.

  ![Assign items](Imagenes/PR_Stock_Control/asignar4.jpg)

- Option **Assign material**. This function allows materials to be automatically assigned from the project defined for stock management.

  ![Assign items](Imagenes/PR_Stock_Control/asignar5.jpg)
...
### 2.4 Stock replenishment
 ...
 - Materials marked in **red** are below the **minimum** or below the **typical** threshold.
    ...
    - **Units pending** shows the quantity pending to be received.  
...
## 8. Managing Offcuts in Logikal

Managing offcuts in Logikal allows reusing leftover profiles, optimizing material usage, and reducing waste. There are several ways to manage offcuts, detailed below.

---

### 8.1. Import Offcuts from Excel

1. From **ENBLAU**, go to **Warehouses → Stock → Detailed** and filter the list by **offcuts**.

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales.jpg)

   > ℹ️ **Note:** It is recommended to apply the **assigned project** filter to consider only the offcuts that should be used.

2. In the menu **Export → Export Profiles**, save the Excel file to a local Windows path.

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales2.jpg)

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales3.jpg)

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales4.jpg)

3. In **Logikal**, open the project where the offcuts will be used. Go to **Project → Offcuts** to open the **Edit Offcuts** window.

4. Select **Import** and load the Excel file of offcuts previously exported from ENBLAU.

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales5.jpg)

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales6.jpg)

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales7.jpg)

   > ℹ️ **Note:** References shown in **red** indicate that the profile does not exist in the current project.

5. In the **Edit Offcuts** window, check **Use Offcuts** and save the changes.

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales8.jpg)
   
   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales9.jpg)

6. The **Cut Optimization** report will show the offcuts available for each profile.

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales10.jpg)

---

### 8.2. Add Offcuts Manually

In Logikal, in the **Edit Offcuts** window, the **Add** option allows manually entering offcuts by specifying:

- Profile reference (existing in the project)
- Quantity
- Length

![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales1.jpg)

---

### 8.3. Manage Offcuts from ENBLAU

1. From **ENBLAU**, in a sales document, go to:  
   **Logikal → Project Editor → Preferences → Offcuts**

   This will open the same **Edit Offcuts** window in Logikal.

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales11.jpg)

   > ℹ️ **Note:** The operation is exactly the same as described previously in Logikal.

2. When listing material from the ENBLAU document, the length of the offcut to be used for the project profiles will be shown.

   ![Managing offcuts](Imagenes/PR_Stock_Control/utilizar_retales12.jpg)

   > ℹ️ **Note:** If the material is assigned, for example, to a **Stock** project, it must be assigned to the project where it will be used.

---

## 9. Stock control and reporting
